export class TCFCommands {
  public static readonly PING: 'ping' = 'ping';
  public static readonly GET_TC_DATA: 'getTCData' = 'getTCData';
  public static readonly GET_IN_APP_TC_DATA: 'getInAppTCData' =
    'getInAppTCData';
  public static readonly GET_VENDOR_LIST: 'getVendorList' = 'getVendorList';
  public static readonly ADD_EVENT_LISTENER: 'addEventListener' =
    'addEventListener';
  public static readonly REMOVE_EVENT_LISTENER: 'removeEventListener' =
    'removeEventListener';
}
