...
describe('CmpApi', (): void => {
  ...
  const assertStub = (): void => {
    expect(
      window[API_FUNCTION_NAME],
      `window.${API_FUNCTION_NAME} while stub`
    ).to.be.a('function');
    ...
    window[API_FUNCTION_NAME]('ping', 2, (ping: Ping): void => {
      ...
      expect(ping.displayStatus, 'ping.displayStatus with stub').to.be
        .undefined;
  ...
  it('should override the stub', (done: () => void): void => {
    ...
    expect(
      window[API_FUNCTION_NAME],
      `window.${API_FUNCTION_NAME} after cmpApi created`
    ).to.be.a('function');
    window[API_FUNCTION_NAME]('ping', 2, (ping: Ping): void => {
      expect(ping.cmpId, 'ping.cmpId after cmpApi created').to.equal(cmpId);
      expect(ping.cmpVersion, 'ping.cmpVersion after cmpApi created').to.equal(
        cmpVersion
      );
      expect(ping.cmpLoaded, 'ping.cmpLoaded after cmpApi created').to.be.true;
      expect(ping.gdprApplies, 'ping.gdprApplies after cmpApi created').to.be
        .undefined;
      expect(ping.cmpStatus, 'ping.cmpStatus after cmpApi created').to.equal(
        CmpStatus.LOADING
      );
      expect(ping.apiVersion, 'ping.apiVersion after cmpApi created').to.equal(
        2
      );
      expect(
        ping.displayStatus,
        'ping.displayStatus after cmpApi created'
      ).to.equal(DisplayStatus.HIDDEN);
      expect(ping.gvlVersion, 'ping.gvlVersion after cmpApi created').to.be
        .undefined;
    });
  ...
  it('should set CmpApiModel.uiVisible to passed in value when CmpApi.uiVisible is set', (done: () => void): void => {
    ...
    expect(CmpApiModel.uiVisible, 'CmpApiModel.uiVisible - initial').to.be
      .false;

    cmpApi.uiVisible = true;

    expect(
      CmpApiModel.uiVisible,
      'CmpApiModel.uiVisible after set cmpApi.uiVisible to true'
    ).to.be.true;
    ...
    expect(
      CmpApiModel.uiVisible,
      'CmpApiModel.uiVisible after set cmpApi.uiVisible to false'
    ).to.be.false;
  ...
  it('should set CmpApiModel.dsiabled to true when CmpApi.disable() is called', (done: () => void): void => {
    ...
    expect(
      CmpApiModel.disabled,
      'CmpApiModel.disabled - after cmpApi.disable() is called'
    ).to.be.true;
  ...
  const runFailCommand = (command: any, version: any, because = ''): void => {
    it(`should callback with success=false and result=error message if command=${command} and version=${version} because ${because}`, (done: () => void): void => {
      ...
      window[API_FUNCTION_NAME](
        command,
        version,
        (result: string, success: boolean): void => {
