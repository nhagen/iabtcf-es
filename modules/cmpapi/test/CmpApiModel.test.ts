...
describe('CmpApiModel', (): void => {
  const assertDefault = (): void => {
    ...
    expect(
      CmpApiModel.tcfPolicyVersion,
      'assert default tcfPolicyVersion'
    ).to.equal(2);
    expect(CmpApiModel.cmpStatus, 'assert default cmpStatus').to.equal(
      CmpStatus.LOADING
    );
    expect(CmpApiModel.displayStatus, 'assert default displayStatus').to.equal(
    ...
    expect(CmpApiModel.gdprApplies, 'assert default gdprApplies').to.be
      .undefined;
    expect(CmpApiModel.eventStatus, 'assert default eventStatus').to.be
      .undefined;
  ...
  it(`should set displayStatus to "${DisplayStatus.VISIBLE}" and eventStatus to "${EventStatus.CMP_UI_SHOWN}" when uiVisible is set to true`, (done: () => void): void => {
    ...
    expect(CmpApiModel.displayStatus, 'displayStatus after').to.equal(
      DisplayStatus.VISIBLE
    );
    expect(CmpApiModel.eventStatus, 'eventStatus after').to.equal(
  ...
  it(`should set gdprApplies to true, displayStatus to "${DisplayStatus.HIDDEN}", eventStatus to "${EventStatus.TC_LOADED}", cmpStatus to "${CmpStatus.LOADED} when tcModel is set for the first time`, (done: () => void): void => {
    ...
    expect(
      CmpApiModel.tcModel,
      'tcModel after (should be a clone)'
    ).not.to.equal(tcModel);
