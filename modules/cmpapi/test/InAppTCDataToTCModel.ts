...
export class InAppTCDataToTCModel {
  private static checkVectorToBitField(
    name: string,
    vector: Vector,
    str: string
  ): void {
  ...
  public static equal(vendorsList?: number[]): void {
    ...
    expect(inAppTCData.eventStatus, 'eventStatus').to.equal(
      CmpApiModel.eventStatus
    );
    expect(inAppTCData.isServiceSpecific, 'isServiceSpecific').to.equal(
      tcModel.isServiceSpecific
    );
    expect(inAppTCData.useNonStandardStacks, 'useNonStandardStacks').to.equal(
      tcModel.useNonStandardStacks
    );
    expect(inAppTCData.purposeOneTreatment, 'purposeOneTreatment').to.equal(
      tcModel.purposeOneTreatment
    );
    expect(inAppTCData.publisherCC, 'publisherCC').to.equal(
      tcModel.publisherCountryCode
    );
    expect(inAppTCData.outOfBand, 'outOfBand').to.be.undefined;

    this.checkVectorToBitField(
      'purpose.consents',
      tcModel.purposeConsents,
      inAppTCData.purpose.consents as string
    );
    this.checkVectorToBitField(
      'purpose.legitimateInterests',
      tcModel.purposeLegitimateInterest,
      inAppTCData.purpose.legitimateInterests as string
    );

    this.checkVectorToBitField(
      'vendor.consents',
      tcModel.vendorConsents,
      inAppTCData.vendor.consents as string
    );
    this.checkVectorToBitField(
      'vendor.legitimateInterests',
      tcModel.vendorLegitimateInterest,
      inAppTCData.vendor.legitimateInterests as string
    );

    this.checkVectorToBitField(
      'specialFeatureOptIns',
      tcModel.specialFeatureOptIns,
      inAppTCData.specialFeatureOptins as string
    );

    this.checkVectorToBitField(
      'publisher.consents',
      tcModel.publisherConsents,
      inAppTCData.publisher.consents as string
    );
    this.checkVectorToBitField(
      'publisher.legitimateInterests',
      tcModel.publisherLegitimateInterest,
      inAppTCData.publisher.legitimateInterests as string
    );
    this.checkVectorToBitField(
      'publisher.customPurpose.consents',
      tcModel.publisherCustomConsents,
      inAppTCData.publisher.customPurpose.consents as string
    );
    this.checkVectorToBitField(
      'publisher.customPurpose.legitimateInterests',
      tcModel.publisherCustomLegitimateInterest,
      inAppTCData.publisher.customPurpose.legitimateInterests as string
    );

    if (tcModel.publisherRestrictions.numRestrictions) {
      expect(
        inAppTCData.publisher.restrictions,
        'inAppTCData.publisher.restrictions'
      ).not.to.be.undefined;
      ...
      purposeIds.forEach((purposeId: number): void => {
        ...
        expect(typeof purposeVector, `restrictions[${strPurposeID}]`).to.equal(
          'string'
        );
        expect(purposeVector.length, 'Restrictions keys length').to.equal(
          maxVendorId
        );
        ...
        for (let i = 0; i < maxVendorId; i++) {
          ...
          const vendorId = i + 1;
          const restrictionType = prVector.getRestrictionType(
            vendorId,
            purposeId
          );
