...
describe('command->CommandMap', (): void => {
  it('has these commands', (done: () => void): void => {
    ...
    expect(typeof CommandMap.getInAppTCData).to.equal(
      typeof GetInAppTCDataCommand
    );
    expect(typeof CommandMap.getVendorList).to.equal(
      typeof GetVendorListCommand
    );
    expect(typeof CommandMap.addEventListener).to.equal(
      typeof AddEventListenerCommand
    );
    expect(typeof CommandMap.removeEventListener).to.equal(
      typeof RemoveEventListenerCommand
    );
