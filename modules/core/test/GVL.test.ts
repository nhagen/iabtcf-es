...
describe('GVL', (): void => {
  const assertPopulated = (gvl: GVL): void => {
    gvlKeys.forEach((key: string): void => {
      ...
      if (key === 'lastUpdated') {
        expect((gvl[key] as Date).getTime(), msg).to.equal(
          new Date(vendorlistJson.lastUpdated).getTime()
        );
  ...
  it('should get latest GVL if nothing is passed to the constructor', (done): void => {
    ...
    req.respond(
      200,
      XMLHttpTestTools.JSON_HEADER,
      JSON.stringify(vendorlistJson)
    );
  });
  ...
  it('should get versioned GVL if version number is passed', (done): void => {
    ...
    expect(req.url).to.equal(
      `${GVL.baseUrl}archives/vendor-list-v${version}.json`
    );
  ...
  it('should narrow a group of vendors when narrowVendorsTo is called with list of ids', (): void => {
    if (Object.keys(vendorlistJson.vendors).length > 1) {
      ...
      expect(gvl.vendors[onlyVendorId]).to.deep.equal(
        vendorlistJson.vendors[onlyVendorId]
      );
      expect(gvl.vendors[Object.keys(vendorlistJson.vendors)[1]]).to.be
        .undefined;
    }
  });
  ...
  it('should replace the language when changeLanguage() is called with valid language', (done: () => void): void => {
    ...
    gvl
      ...
      .then((): void => {
        expect(gvl.purposes, 'purposes should match').to.deep.equal(
          translationJson.purposes
        );
        expect(
          gvl.specialPurposes,
          'specialPurposes should match'
        ).to.deep.equal(translationJson.specialPurposes);
        expect(gvl.features, 'features should match').to.deep.equal(
          translationJson.features
        );
        expect(
          gvl.specialFeatures,
          'specialFeatures should match'
        ).to.deep.equal(translationJson.specialFeatures);
        expect(gvl.stacks, 'stacks should match').to.deep.equal(
          translationJson.stacks
        );

        expect(gvl.purposes, 'purposes should match').to.not.deep.equal(
          vendorlistJson.purposes
        );
        expect(
          gvl.specialPurposes,
          'specialPurposes should match'
        ).to.not.deep.equal(vendorlistJson.specialPurposes);
        expect(gvl.features, 'features should match').to.not.deep.equal(
          vendorlistJson.features
        );
        expect(
          gvl.specialFeatures,
          'specialFeatures should match'
        ).to.not.deep.equal(vendorlistJson.specialFeatures);
    ...
    expect(req.method).to.equal('GET');
    expect(req.url).to.equal(
      GVL.baseUrl + GVL.languageFilename.replace('[LANG]', language)
    );
  ...
  const langNotOk = (language: string): void => {
    it(`should throw an error if ${language} is passed to changeLanguage()`, (done: () => void): void => {
      ...
      gvl.changeLanguage(language).catch((err): void => {
  ...
  it(`should throw an error if GVL.baseUrl isn't set before changeLaguage() is called`, (done: () => void): void => {
    ...
    gvl
      ...
      .then((): void => {
        expect.fail(
          `without setting GVL.baseURL, this should have failed: ${GVL.baseUrl}`
        );
        done();
      })
      .catch((err): void => {
  ...
  const vendorGroupGoodTest = (
    purposeOrFeature: string,
    subType: string,
    special: boolean
  ): void => {
    it(`should group vendors by${
      special ? ' special' : ''
    } ${purposeOrFeature} ${subType}`, (): void => {
      GVL.baseUrl = 'http://sweetcmp.com';
      ...
      Object.keys(gvl[gvlKey]).forEach((id: string): void => {
        ...
        const gvlMethodName: string =
          'getVendorsWith' + specialOrSubType + cappedPORF;
        const gvlMap: IntMap<Vendor> = gvl[gvlMethodName](intId);

        Object.keys(vendorlistJson.vendors).forEach(
          (vendorId: string): void => {
            ...
            if (values && values.includes(intId)) {
              expect(
                gvlMap[vendorId],
                `vendorId ${vendorId} should be in the map when ${gvlMethodName}() is called`
              ).to.not.be.undefined;
