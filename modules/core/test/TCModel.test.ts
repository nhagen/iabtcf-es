...
describe('TCModel', (): void => {
    ...
    const shouldBeNotOk = (valueToTest: number): void => {
      it(`should not allow ${fieldName} to be ${valueToTest}`, (): void => {
        ...
        try {
          ...
          expect.fail(
            `${fieldName}=${valueToTest} should have thrown an error`
          );
        } catch (error) {
  ...
  const testDate = (fieldName: string): void => {
    describe(fieldName, (): void => {
      runValidTest(fieldName, (): void => {
        ...
        expect((tcModel[fieldName] as Date).getTime()).to.equal(
          Math.round(testDate.getTime())
        );
      });
    });
  };
  ...
  const testBoolean = (fieldName: string): void => {
    describe(fieldName, (): void => {
      runValidTest(fieldName, (): void => {
        ...
        expect(tcModel[fieldName], `${fieldName} should be default false`).to.be
          .false;

        tcModel[fieldName] = true;

        expect(tcModel[fieldName], `${fieldName} should have been set to true`)
          .to.be.true;
      });
    });
  };

  const testInstanceOf = <T>(
    fieldName: string,
    theInstanceOf: { new (): T }
  ): void => {
    ...
    describe(fieldName, (): void => {
      const gvl: GVL = new GVL(
        require('@iabtcf/testing/lib/vendorlist/vendor-list.json')
      );
  ...
  it('should construct a TCModel with a GVL argument', (done: () => void): void => {
    ...
    tcModel.gvl.readyPromise.then((): void => {
      expect(
        tcModel.policyVersion,
        'policyVersion should be picked up from gvl'
      ).to.equal(gvl.tcfPolicyVersion);
      expect(
        tcModel.vendorListVersion,
        'vendorListVersion should be picked up from gvl'
      ).to.equal(gvl.vendorListVersion);
      expect(tcModel.gvl, 'should save and make accessible the gvl').to.equal(
        gvl
      );
      done();
    });
  ...
  const runSetAllAndUnsetAll = (): void => {
    ...
    for (const vectorName in setUnSetAlls) {
      if (setUnSetAlls.hasOwnProperty(vectorName)) {
        const endOfFunctionName =
          'All' + vectorName.charAt(0).toUpperCase() + vectorName.slice(1);
