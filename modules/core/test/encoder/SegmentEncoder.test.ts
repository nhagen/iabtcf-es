...
describe('encoder->SegmentEncoder', (): void => {
  ...
  it('TCModel->OOBAllowed segment->TCModel and should be equal', (done: () => void): void => {
    ...
    const strToInt: (str: string) => number = (str: string): number =>
      parseInt(str, 10);
    const allowedVendors: number[] = Object.keys(
      tcModel.gvl.getVendorsWithConsentPurpose(1)
    ).map(strToInt);
    ...
    tcModel.gvl.readyPromise.then((): void => {
      ...
      expect(decodedModel.vendorsAllowed.size).to.equal(
        tcModel.vendorsAllowed.size
      );
      done();
    });
  });
