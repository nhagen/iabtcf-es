...
describe('encoder/field->PurposeRestrictionVectorEncoder', (): void => {
  ...
  it('should create one restriction group and two vendor ranges', (): void => {
    ...
    const numRestrictions: number = IntEncoder.decode(
      encoded.substr(index, BitLength.numRestrictions)
    );
    ...
    // purposeId
    const purpId: number = IntEncoder.decode(
      encoded.substr(index, BitLength.purposeId)
    );
    ...
    // restrictionType
    const restrictionType: number = IntEncoder.decode(
      encoded.substr(index, BitLength.restrictionType)
    );
    ...
    const numEntries: number = IntEncoder.decode(
      encoded.substr(index, BitLength.numEntries)
    );
  ...
  it('should encode and decode and have the same output', (): void => {
    ...
    for (let i = 1; i <= vendorLength; i++) {
      prVector.add(
        i,
        new PurposeRestriction(makeRandomInt(1, 12), makeRandomInt(0, 2))
      );
    }

    const encoded: string = PurposeRestrictionVectorEncoder.encode(prVector);
    prVector.bitLength = encoded.length;
    const decodedPRV: PurposeRestrictionVector = PurposeRestrictionVectorEncoder.decode(
      encoded
    );
