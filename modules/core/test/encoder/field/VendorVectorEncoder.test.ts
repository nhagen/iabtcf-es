...
describe('encoder/field->VendorVectorEncoder', (): void => {
  ...
  describe('encode', (): void => {
    ...
    it(`should encode a vector of length 10 with no gaps as a bitfield`, (): void => {
      ...
      expect(result.length, 'length is HEADER_LENGTH + numVendors').to.equal(
        HEADER_LENGTH + numVendors
      );

      // first bits should be the max id
      expect(result.substr(0, BitLength.maxId), `starts with maxId`).to.equal(
        pad(numVendors.toString(2), BitLength.maxId)
      );
      expect(
        result.substr(BitLength.maxId, 1),
        'BitField encoding type is 0'
      ).to.equal('0');
      ...
      for (let i = 0; i < bitField.length; i++) {
        expect(
          bitField[i],
          `vendor #${i + 1} should be 1 if true, 0 if false at index ${i}`
        ).to.equal(vector.has(i + 1) ? '1' : '0');
      }
    });
    /**
    ...
    it(`should encode a vector with 46 vendors and no gaps as a range`, (): void => {
      ...
      expect(result.length, 'length: header + num entries + 1 range').to.equal(
        HEADER_LENGTH + RANGE_NUM_ENTRIES + SINGLE_RANGE
      );
      ...
      // first bits should be the max id
      expect(
        parseInt(result.substr(index, BitLength.maxId), 2),
        'first bits should be maxId'
      ).to.equal(numVendors);

      index += BitLength.maxId;

      expect(
        result.substr(index, BitLength.encodingType),
        'should be a range encoding ie. 1'
      ...
      expect(parseInt(result.substr(index, RANGE_NUM_ENTRIES), 2)).to.equal(1);
      ...
      expect(result.substr(index, BitLength.vendorId)).to.equal(
        pad('1', BitLength.vendorId)
      );
      index += BitLength.vendorId;

      // second ID should be numVendors
      expect(result.substr(index, BitLength.vendorId)).to.equal(
        pad(numVendors.toString(2) + '', BitLength.vendorId)
      );
    });
    ...
    it('should encode a vector of length 48 1 gap as a bitfield since a that would be shorter', (): void => {
      ...
      // first bits should be the max id
      expect(result.substr(0, BitLength.maxId)).to.equal(
        pad(numVendors.toString(2), BitLength.maxId)
      );
    ...
    it('should encode a vector of length 100 1 gap as a range since a that would be shorter', (): void => {
      ...
      expect(
        result.length,
        `length of the result should be ${HEADER_LENGTH +
          RANGE_NUM_ENTRIES +
          SINGLE_RANGE * 2}`
      ...
      expect(
        parseInt(result.substr(index, BitLength.maxId), 2),
        `maxId should be ${vector.maxId}`
      ).to.equal(vector.maxId);

      index += BitLength.maxId;

      // encoding type 1 = range, 0 = bitfield
      expect(result.substr(index, 1), 'Encoding type should be 1').to.equal(
        '1'
      );
      index += 1;

      expect(
        parseInt(result.substr(index, RANGE_NUM_ENTRIES), 2),
        `numEntries should be 2`
      ).to.equal(2);
      ...
      expect(
        ...
        'first singeOrRange should be 1'
      ).to.equal('1');
      index += 1;

      // first ID should be 1
      expect(
        parseInt(result.substr(index, BitLength.vendorId), 2),
        'first range first id should be 1'
      ).to.equal(1);
      ...
      expect(
        parseInt(result.substr(index, BitLength.vendorId), 2),
        `first range last id should be ${vendorWithNo - 1}`
      ...
      expect(
        result.substr(index, 1),
        'second singeOrRange should be 1'
      ).to.equal('1');
      index += 1;
      ...
      expect(
        parseInt(result.substr(index, BitLength.vendorId), 2),
        `second range first id should be ${vendorWithNo + 1}`
      ).to.equal(vendorWithNo + 1);
      index += BitLength.vendorId; // +16
      ...
      expect(
        parseInt(result.substr(index, BitLength.vendorId), 2),
        `second range last id should be ${numVendors}`
      ).to.equal(numVendors);
      index += BitLength.vendorId; // +16
    });
  });
