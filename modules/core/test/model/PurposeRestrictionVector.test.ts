...
export function run(): void {
  describe('PurposeRestrictionVector', (): void => {
    ...
    it('should store vendors by purpose restriction in order', (): void => {
      ...
      const expected: number[] = vendors.sort(
    ...
    it("should return an empty array if a purpose restriction hasen't been defined for a vendor", (): void => {
      ...
      expect(
        prVector.getVendors(purposeRestriction),
        'prVector.getVendors(purposeRestriction)'
      ).to.be.empty;
    });
    ...
    it('should return all purposes restrictions set', (): void => {
      ...
      const result: string[] = prv
        .getRestrictions()
        .map((pr: PurposeRestriction): string => {
          return pr.hash;
        })
        .sort();
    ...
    it('should return the restriction set on a particular vendor', (): void => {
      ...
      const prvRestrictions: PurposeRestriction[] = prv.getRestrictions(
        vendorId
      );
    ...
    it('should set restriction type 0 on vendor regardless of flexible legal basis', (): void => {
      ...
      if (!chosenVendorId) {
        throw new Error(
          'No sutable vendor was found without a flexible purpose id for ' +
            purposeId
        );
      }
    ...
    it('should not set restriction type 1 on vendor if no flexible legal basis for vendor', (): void => {
      ...
      vendorIds.forEach((id: string): void => {
        if (
          !vendorlistJson.vendors[id].flexiblePurposes.includes(purposeId) &&
      ...
      if (!chosenVendorId) {
        throw new Error(
          'No sutable vendor was found with a flexible and legInt purpose id for ' +
            purposeId
        );
      }
